                ...
                <div class="card-body">
                    <form method="POST" action="{{ route('visits.update', $visit->id) }}">
                            ...
                            <div class="col-md-6">
                                <select id="broker_id" name="broker_id" class="form-select select2">
                                    @foreach($brokers as $broker)
                                        <option value="{{ $broker->id }}" {{ $visit->broker_id == $broker->id ? 'selected' : '' }}>
                                            {{ $broker->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-6">
                                <select id="client_id" name="client_id" class="form-select select2">
                                    @foreach($clients as $client)
                                        <option value="{{ $client->id }}" {{ $visit->client_id == $client->id ? 'selected' : '' }}>
                                            {{ $client->name }}
                                        </option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-md-6">
                                <select id="propertie_id" name="propertie_id" class="form-select select2">
                                    @foreach($properties as $property)
                                        <option value="{{ $property->id }}" {{ $visit->propertie_id == $property->id ? 'selected' : '' }}>
                                            {{ $property->title }}
                                        </option>
                                    @endforeach
                                </select>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-6">
                                <input type="datetime-local" class="form-control" id="date" name="date" required value="{{ $visit->date }}">
                            </div>
                        </div>
                        ...
                        <div class="row mb-3">
                            ...
                            <div class="col-md-6">
                                <select id="type" name="type" class="form-select" required>
                                    <option value="Agendado" {{ $visit->type == 'Agendado' ? 'selected' : '' }}>Agendado</option>
                                    <option value="Não agendado" {{ $visit->type == 'Não agendado' ? 'selected' : '' }}>Não agendado</option>
                                    <option value="Realizado" {{ $visit->type == 'Realizado' ? 'selected' : '' }}>Realizado</option>
                                </select>
                            </div>
                        </div>
